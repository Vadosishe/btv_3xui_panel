...
│   │   │   └── layout.tsx     # Сайдбар с отображением версий (BTV Panel v1.3.12)
...
│   │   ├── services/          # [NEW] Сервисный слой бизнес-логики (Mini-API)
│   │   │   ├── client-service.ts   # Транзакции клиентов, 3XUI, лимиты, генерация ссылок подписки
│   │   │   ├── company-service.ts  # Управление компаниями, 3XUI группы, каскадная очистка
│   │   │   └── template-service.ts # Тарифные шаблоны, привязки нод, Amnezia WG связи
...
### 5. Сервисный слой бизнес-логики (Service Layer / Mini-API)
Для исключения "спагетти-кода" и перегрузки роут-хендлеров (Controllers), вся логика работы с внешними API, транзакциями БД и аудит-логированием вынесена в изолированные сервисные классы (`src/lib/services/`):
* **Controllers (`route.ts`)** должны быть максимально тонкими: они занимаются исключительно валидацией HTTP-запроса, авторизацией сессии, вызовом методов сервисного слоя и возвратом стандартизированного `NextResponse`.
* **Services (`*-service.ts`)** инкапсулируют бизнес-логику и методы манипуляции данными. Они не знают о деталях HTTP-запросов (NextRequest/NextResponse) и оперируют чистыми типами данных, что делает их модульными, тестируемыми и переносимыми.
...
3. Проверить в сайдбаре панели изменение номера версии (например, до `BTV Panel v1.3.12`).
